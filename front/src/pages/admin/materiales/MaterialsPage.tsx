...
import { HiPlus, HiOutlinePencil, HiOutlineTrash, HiOutlineCube, HiChevronUp, HiChevronDown } from 'react-icons/hi2';
...
export function MaterialsPage() {
  ...
  const filteredMateriales = useMemo(() => {
    ...
    return materiales.filter(m => {
      const codigoMatch = m.codigo?.toLowerCase().includes(query);
      const nombreMatch = m.nombre?.toLowerCase().includes(query);
      const descMatch = m.descripcion?.toLowerCase().includes(query);
      const unidadMatch = m.unidad?.toLowerCase().includes(query);
      return codigoMatch || nombreMatch || descMatch || unidadMatch;
    });
  }, [materiales, searchQuery]);

  // Sort Logic
  const sortedMateriales = useMemo(() => {
    if (!sortField) return filteredMateriales;

    return [...filteredMateriales].sort((a, b) => {
      let valA: any = '';
      let valB: any = '';
  ...
  }, [filteredMateriales, sortField, sortDirection]);

  // Pagination Logic
  const paginatedMateriales = useMemo(() => {
    const startIndex = (currentPage - 1) * itemsPerPage;
    return sortedMateriales.slice(startIndex, startIndex + itemsPerPage);
  }, [sortedMateriales, currentPage, itemsPerPage]);
    ...
    <MainLayout title="Gestión de Materiales">
      <div className="space-y-6">
        ...
        <div className="bg-white border border-slate-200 rounded-2xl overflow-hidden shadow-sm">
              ...
              <div className="flex flex-wrap items-center gap-4 justify-between sm:justify-end w-full sm:w-auto">
                ...
                <span className="text-xs text-slate-400 font-semibold uppercase tracking-wider">
                  {filteredMateriales.length === 0 ? (
                    ...
                    <span>Total: {filteredMateriales.length}</span>
                  )}
                </span>
              </div>
            </div>
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-slate-50 border-b border-slate-200">
                  ...
                  <th className="px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider select-none text-center">Unidad</th>
                  <th className="px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider select-none text-right">Coste</th>
              ...
              <tbody className="divide-y divide-slate-100">
                {isLoading ? (
                  ...
                  <tr>
                    <td colSpan={5} className="px-6 py-10 text-center text-slate-400 italic">No hay materiales registrados.</td>
                  </tr>
                ) : (
                  paginatedMateriales.map((m) => (
                      ...
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-3">
                          <div>
                            ...
                            <div className="text-xs text-slate-400 truncate max-w-[200px]">{m.descripcion || '---'}</div>
                          </div>
                        </div>
                      </td>
                      <td className="px-6 py-4 text-sm font-mono text-slate-500 text-center">{m.unidad}</td>
                      <td className="px-6 py-4 text-right">
                        <span className="px-1.5 py-0.5 rounded bg-brand-50 text-brand-600 text-[10px] font-medium border border-brand-100">
          ...
          {/* Pagination Controls */}
          {!isLoading && sortedMateriales.length > 0 && (
